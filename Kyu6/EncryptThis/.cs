...
public class Kata
{
    public static string EncryptThis(string input)
    {
        return string.Join(" ", input.Split(' ').Select(word =>
        {
            ...
            string result = ((int)word[0]).ToString();
            if (word.Length == 1) return result;
            if (word.Length == 2) return result + word[1];
            return result + word[word.Length - 1] + word.Substring(2, word.Length - 3) + word[1];
        }));
    }
}
...
public class ExampleTests
{
    ...
    {
        Assert.That(Kata.EncryptThis(input), Is.EqualTo(expected));
    }
}